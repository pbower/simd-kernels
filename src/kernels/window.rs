...
{
    let n = data.len();
    assert_eq!(n, out.len(), "rolling_push_pop_to: input/output length mismatch");
    ...
    FRem: FnMut(T, T) -> T,
{
    ...
    rolling_push_pop_to(data, mask, subwindow, add, remove, zero, &mut out, &mut out_mask);
    (out, out_mask)
}
...
{
    let n = data.len();
    assert_eq!(n, out.len(), "rolling_extreme_to: input/output length mismatch");
...
pub fn rolling_product_int_to<T: Num + Copy + One + Zero>(
    ...
    rolling_push_pop_to(data, mask, subwindow, |a, b| a * b, |a, b| a / b, T::one(), out, out_mask);
}
...
pub fn rolling_product_bool_to(
    ...
    let n = data.len();
    for i in 0..n {
        let start = if i + 1 >= subwindow { i + 1 - subwindow } else { 0 };
        let mut acc = true;
...
pub fn rolling_mean_float_to<T: Float + Copy + Zero>(
    ...
    for i in 0..n {
        ...
        unsafe { out_mask.set_unchecked(i, valid) };
        out[i] = if valid { sum / T::from(subwindow as u32).unwrap() } else { T::zero() };
    }
    if subwindow > 0 && subwindow - 1 < out.len() {
        unsafe { out_mask.set_unchecked(subwindow - 1, false) };
        out[subwindow - 1] = T::zero();
...
pub fn rolling_min_int_to<T: Ord + Copy + Zero>(
    ...
    rolling_extreme_to(data, mask, subwindow, |a, b| a < b, T::zero(), out, out_mask);
    if subwindow > 0 && subwindow - 1 < out.len() {
        out_mask.set(subwindow - 1, false);
        out[subwindow - 1] = T::zero();
    }
...
fn shift_with_bounds_to<T: Copy>(
    ...
    assert_eq!(len, out.len(), "shift_with_bounds_to: input/output length mismatch");
    for i in 0..len {
...
pub fn lag_int_to<T: Copy + Default>(
    ...
    let len = data.len();
    shift_with_bounds_to(data, mask, len, |i| if i >= n { Some(i - n) } else { None }, T::default(), out, out_mask);
}
...
pub fn lead_int_to<T: Copy + Default>(
    ...
    let len = data.len();
    shift_with_bounds_to(data, mask, len, |i| if i + n < len { Some(i + n) } else { None }, T::default(), out, out_mask);
}
...
pub fn lag_float_to<T: Copy + num_traits::Zero>(
    ...
    let len = data.len();
    shift_with_bounds_to(data, mask, len, |i| if i >= n { Some(i - n) } else { None }, T::zero(), out, out_mask);
}
