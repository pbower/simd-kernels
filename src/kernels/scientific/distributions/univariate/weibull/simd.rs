...
pub fn weibull_pdf_simd(
    ...
    if !(shape > 0.0 && shape.is_finite()) || !(scale > 0.0 && scale.is_finite()) {
        return Err(KernelError::InvalidArguments("weibull_pdf: invalid shape or scale".into()));
    }
    if x.is_empty() {
    ...
    if !has_nulls(null_count, null_mask) {
        if is_simd_aligned(x) {
            let (data, mask) =
                dense_univariate_kernel_f64_simd::<N, _, _>(x, null_mask.is_some(), simd_body, scalar_body);
            return Ok(FloatArray { data: data.into(), null_mask: mask });
        } else {
            let (data, mask) =
                dense_univariate_kernel_f64_std(x, null_mask.is_some(), scalar_body);
            return Ok(FloatArray { data: data.into(), null_mask: mask });
        }
    }
    ...
    if is_simd_aligned(x) {
        ...
        Ok(FloatArray { data: data.into(), null_mask: Some(out_mask) })
    } else {
        let (data, out_mask) =
            masked_univariate_kernel_f64_std(x, mask_ref, scalar_body);
        Ok(FloatArray { data: data.into(), null_mask: Some(out_mask) })
    }
}
...
pub fn weibull_cdf_simd(
    ...
    if !(shape > 0.0 && shape.is_finite()) || !(scale > 0.0 && scale.is_finite()) {
        return Err(KernelError::InvalidArguments("weibull_cdf: invalid shape or scale".into()));
    }
    if x.is_empty() {
        return Ok(FloatArray::from_slice(&[]));
    }
    ...
    let scalar_body = move |xi: f64| -> f64 {
        if xi < 0.0 { 0.0 } else { 1.0 - (-(xi * inv_scale).powf(shape)).exp() }
    };
...
pub fn weibull_quantile_simd(
    ...
    if !(shape > 0.0 && shape.is_finite()) || !(scale > 0.0 && scale.is_finite()) {
        return Err(KernelError::InvalidArguments("weibull_quantile: invalid shape or scale".into()));
    }
    if p.is_empty() {
        return Ok(FloatArray::from_slice(&[]));
    }
    ...
    if !has_nulls(null_count, null_mask) {
        if is_simd_aligned(p) {
            let (data, mask) =
                dense_univariate_kernel_f64_simd::<N, _, _>(p, null_mask.is_some(), simd_body, scalar_body);
            return Ok(FloatArray { data: data.into(), null_mask: mask });
        } else {
            let (data, mask) =
                dense_univariate_kernel_f64_std(p, null_mask.is_some(), scalar_body);
            return Ok(FloatArray { data: data.into(), null_mask: mask });
        }
    }
    ...
    if is_simd_aligned(p) {
        ...
        let (data, out_mask) =
            masked_univariate_kernel_f64_std(p, mask_ref, scalar_body);
        Ok(FloatArray { data: data.into(), null_mask: Some(out_mask) })
    }
}
